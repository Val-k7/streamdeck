...
describe('E2E - Scénarios Utilisateur', () => {
  ...
  beforeAll(async () => {
    if (!HANDSHAKE_SECRET) {
      console.warn('HANDSHAKE_SECRET not provided, skipping authenticated WS tests')
      return
    }

    try {
      const response = await fetch(
        `${SERVER_URL}/tokens/handshake?secret=${encodeURIComponent(HANDSHAKE_SECRET)}&clientId=e2e-test-client`,
        { method: 'POST' }
      )
      if (response.ok) {
  ...
  describe('Scénario 1: Premier Lancement', () => {
    ...
    it('should complete pairing and connect', async () => {
      // Request pairing
      const requestResponse = await fetch(`${SERVER_URL}/discovery/pairing/request`, {
        method: 'POST',
      })
