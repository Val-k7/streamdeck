...
describe("User Scenarios E2E", () => {
  ...
  describe("Scénario: Premier Lancement", () => {
    it("should complete first launch workflow", async () => {
      ...
      const pairingRequestResponse = await fetch(
        `${serverUrl}/discovery/pairing/request`,
        {
          method: "POST",
        }
      );
  ...
  describe("Scénario: Import/Export de Profil", () => {
    it("should export and import a profile", async () => {
      const createResponse = await fetch(
        `${serverUrl}/profiles/e2e-export-profile`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      const importResponse = await fetch(
        `${serverUrl}/profiles/e2e-imported-profile`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
