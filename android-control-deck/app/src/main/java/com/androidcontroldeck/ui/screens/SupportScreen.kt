        ...
        item {
            Card(modifier = Modifier.fillMaxWidth()) {
                Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Text("Connectivité : ${if (isOnline) "OK" else "À corriger"} | WebSocket : ${if (isSocketConnected) "connecté" else "non connecté"}")
                    Text("• Autorisez le port du serveur dans votre pare-feu (TCP ${'$'}{if (isSocketConnected) "ouvert" else "à ouvrir"}).")
